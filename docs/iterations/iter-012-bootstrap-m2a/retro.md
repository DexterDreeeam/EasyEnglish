...
- **What shipped**: The repo was completely cleared of the C++ working tree (git history keeps v0.3.0) and rebuilt as an m2a-style cargo workspace. 5 top-level crates (Dict / Core / Win / Mac / Linux) + root-level `.design.md` / `.interface.md` / repository instructions / `README.md` / `product.json` / `.cargo/config.toml` / `rust-toolchain.toml` + ADR-0004. `cargo build --workspace` passes in ~8.5 s; `cargo fmt --all --check` / `cargo clippy --workspace --all-targets -- -D warnings` are clean.

- **Where the AI went wrong / lessons**
  1. `cargo nextest` exits 1 with "no tests to run" by default in the Phase 1 empty-lib state. Already changed the command in repository instructions / README to `--no-tests=pass`; once every crate has real tests from iter-013 on, this flag is a no-op.
  ...
  3. Capitalized top-level directories `Dict/Core/Win/Mac/Linux` are uncommon in the Rust ecosystem, but the user explicitly required them. The crate names inside use the Rust-idiomatic `ee-dict` etc., recorded in repository instructions to stop the AI from "correcting" the directory names in a PR.
