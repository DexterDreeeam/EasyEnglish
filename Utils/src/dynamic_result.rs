...
mod tests {
    ...
    #[test]
    fn snapshot_terminal_returns_none_initially() {
        let (r, _p) = DynamicResult::<i32>::new(0);
        assert!(r.inner.snapshot_terminal().is_none());
    }

    #[test]
    fn cancel_winner_freezes_complete() {
        let (r, p) = DynamicResult::<i32>::new(0);
        r.cancel();
        p.complete();
        match r.wait(Some(Duration::from_millis(10))) {
            Signal::Finished => {}
            other => panic!("expected Finished, got {:?}", other),
        }
        assert!(r.inner.cancelled.load(Ordering::Acquire));
        assert!(!r.inner.completed.load(Ordering::Acquire));
    }

    #[test]
    fn complete_winner_freezes_cancel() {
        let (r, p) = DynamicResult::<i32>::new(0);
        p.complete();
        r.cancel();
        assert!(r.inner.completed.load(Ordering::Acquire));
    ...
    #[test]
    fn fail_winner_freezes_others() {
        let (r, p) = DynamicResult::<i32>::new(0);
        p.fail("boom");
        p.complete();
        r.cancel();
        match r.wait(Some(Duration::from_millis(10))) {
            Signal::Failed(msg) => assert_eq!(msg, "boom"),
    ...
    #[test]
    fn payload_to_string_handles_str_slice() {
        let payload: Box<dyn Any + Send> = Box::new("oops");
        assert_eq!(payload_to_string(payload), "oops");
    }

    #[test]
    fn payload_to_string_handles_owned_string() {
        let payload: Box<dyn Any + Send> = Box::new(String::from("oops"));
        assert_eq!(payload_to_string(payload), "oops");
    }

    #[test]
    fn payload_to_string_handles_unknown_type() {
        let payload: Box<dyn Any + Send> = Box::new(42_i32);
        assert_eq!(payload_to_string(payload), "panic with non-string payload");
    }
}
