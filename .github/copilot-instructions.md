...
- Version checks compare only the root `version` file content from the installed package and GitHub raw. Do not include or compare package language suffixes in version checks.
- Windows installer filenames add a language-pair suffix after the version. For the Chinese-English build, use `EasyEnglish-<version>-CN.exe`, where `CN` means Chinese-English bidirectional dictionary. Future language-pair packages may use other suffixes.
...
3. **Tests are Contract**: Any change to `src/` must be paired with matching Rust tests under `ee/Tests/UnitTest/`. UI behavior changes must also update the markdown UI specifications under `ee/Tests/UITest/`.
